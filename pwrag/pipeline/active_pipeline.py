...
from pwrag.utils.utils import get_retriever, get_generator, selfask_pred_parse, ircot_pred_parse
from pwrag.pipeline.pipeline import BasicPipeline
from pwrag.dataset.dataset import get_batch_dataset, merge_batch_dataset
from pwrag.prompt.base_prompt import PromptTemplate


class IterativePipeline(BasicPipeline):

    # inference stage: query -> generator -> retrieval -> generator -> retrieval -> generator ...
    # The query is augmented with the generation result from last iteration, and then sent to retriever for retrieval. 
    # The retrieval result is further augmented to the query and sent to generator for next iteration. 
    # After a fixed number of iterations, the final generation result is returned as the answer.
    ...
    def run_single_sample(self, question):
        past_generation_result = ""
        for iter_idx in range(self.iter_num):
            if iter_idx == 0:
                input_query = question
            else:
                input_query = f"{question} {past_generation_result}"

            # generation-augmented retrieval
            retrieval_result = self.retriever.search(input_query)
            
            # retrieval-augmented generation
            input_prompt = self.prompt_template.get_string(question=question, retrieval_result=retrieval_result)
            past_generation_result = self.generator.generate(input_prompt)[0]

        return past_generation_result


    def run_on_dataset(self, dataset, do_eval=True, pred_process_fun=None):
