...
def parse_query(model_name, query_list, instruction=None, is_query=True):
    ...
    if instruction is not None:
        ...
        instruction = set_default_instruction(model_name, is_query=is_query, is_zh=False)
...
def unwrap_faiss_index(index):
    # Unwrap common FAISS wrappers that store the real index in `.index`
    while hasattr(index, "index"):
        index = index.index
    return index

def inspect_faiss_index(index):
